...
const EditPlayersScreen = ({ navigation, route }: any) => {
	...
	const loadData = async () => {
		try {
			...
			const charsResult =
				await db.getAllAsync<CharacterFull>(
					`SELECT
						id,
						character_name_id,
						player_id,
	...
	const handleSave = async () => {
		...
		// -------------------------------------------------
		// CHECK EACH PLAYER HAS CHARACTER
		// -------------------------------------------------
		...
		for (const player of players) {
			...
			// Новий гравець має id = -1.
			// У такому випадку він ще не буде
			// прив'язаний до characters до Save.
			//
			// Тому для нових гравців перевірку
			// виконуємо окремо нижче через
			// character assignment по позиції.
			...
			if (
				...
				Alert.alert(
					'Помилка',
					`Гравець "${player.name}" не має жодного персонажа`
				);
